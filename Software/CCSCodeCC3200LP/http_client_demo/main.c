...
#define GET_REQUEST_URI 	"/getData"


#define HOST_NAME       	"192.168.1.25" //"<host name>"httpbin.org
...
unsigned int captureButtonEvent = 0;
...
static int HTTPPostMethod(HTTPCli_Handle httpClient)
{
    ...
    HTTPCli_Field fields[4] = {
                                ...
                                {HTTPCli_FIELD_NAME_CONTENT_TYPE, "text/html; charset=iso-8859-1"},
                                {NULL, NULL}
                            };
...
void Button2InterruptHandler()
{
    ...
    captureButtonEvent = 1; //flag control button event
...
int main()
{
    ...
    Button_IF_Init(Button2InterruptHandler,Button3InterruptHandler);
    //--------------------------------------------------------------
...
/*
    ...
    */
    ...
    while(1)
    {
        unsigned int i = 0;
        _SlNonOsMainLoopTask();
        if (captureButtonEvent == 1)
                {
                    for(i = 0; i < 3000000; i++);
                    if (captureButtonEvent == 1)
                    {
                        captureButtonEvent = 0;
                        UART_PRINT("HTTP Get Begin:\n\r");
                        lRetVal = HTTPGetMethod(&httpClient);
                        if(lRetVal < 0)
                        {
